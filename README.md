# Subtitle Extractor for AI

一个 Chrome 插件，用于提取 **B站** 和 **YouTube** 视频字幕，并通过 DOM 暴露数据供 Claude 等支持页面读取的 AI 助手使用。
...
- **Badge 状态指示**：绿色 ✓ 表示字幕可用，红色 ! 表示需要操作
- **国际化支持**：支持中文和英文界面
...
1. 打开有 CC 字幕的视频页面
2. **开启字幕**（点击 CC 按钮）并播放几秒
3. 等待插件图标显示绿色 ✓
4. 点击插件图标，选择语言并提取

> **注意**: YouTube 只会捕获当前激活的字幕语言。如需其他语言，请在播放器中切换字幕（设置 → 字幕），新语言会自动添加到插件中。
